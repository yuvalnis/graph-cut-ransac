...
public:
    ...
    inline static constexpr size_t sampleSize() { return 2; }
...
protected:
    ...
    static constexpr size_t num_sets = Base::ResidualDimension;
...
)
{
    ...
    const auto vp = l1.cross(l2); // intersection of lines is vanishing point
...
ResidualType RectifyingHomographyTwoSIFTSolver::residual(
    const cv::Mat& feature,
    const SIFTRectifyingHomography& model
)
{
...
ResidualType RectifyingHomographyTwoSIFTSolver::squaredResidual(
    const cv::Mat& feature,
    const SIFTRectifyingHomography& model
)
{
...
) const
{
    ...
    auto get_inlier = [&data, &inliers](const size_t& i) {
        const auto* data_ptr = reinterpret_cast<double*>(data.data);
