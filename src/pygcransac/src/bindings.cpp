...
)
{
    ...
    std::vector<bool> cpp_scale_inliers(num_features);
	std::vector<bool> cpp_orientation_inliers(num_features);
	...
	py::array_t<bool> scale_inliers = py::array_t<bool>(num_features);
    ...
    if (num_inliers == 0)
	{
        return py::make_tuple(pybind11::cast<pybind11::none>(Py_None), scale_inliers);
    }
	// construct python array for orientation inliers from C++ vector
	py::array_t<bool> orientation_inliers = py::array_t<bool>(num_features);
    ...
    if (num_inliers == 0)
	{
        return py::make_tuple(pybind11::cast<pybind11::none>(Py_None), orientation_inliers);
    }
	// construct python array for homography for C++ vector
    py::array_t<double> homography = py::array_t<double>({3, 3});
    py::buffer_info homography_buff = homography.request();
